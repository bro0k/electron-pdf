...
function readPrintingOptions(req) {
  ...
  var printDelay = req.get('x-pdf-printDelay');
  ...
  return {
    landscape: landscape,
    pageSize: pageSize && pageSize.startsWith('{') ? JSON.parse(pageSize) : pageSize,
    printBackground: !noBackgrounds,
    marginsType: marginsType,
    printDelay: printDelay && printDelay > 0 ? printDelay : 0
  };
}
...
function printToPdf(url, options) {
  return pool.loadUrl(url, function (window) {
    ...
    return Promise.fromNode(function (callback) {
      if(options.printDelay){
        setTimeout(function (){
          window.webContents.printToPDF(options).then(function (data){
            callback(null,data);
