...
function waitUntilLoaded(window, timeout) {
  ...
  return Promise.fromNode(function (callback) {
    onStopLoading = function (event) {
      callback(null, window)
      if (event && event.reply) {
        console.log('监听到view-ready事件')
        event.reply('view-ready-ack')
      }
    }
    ipcMain.once('view-ready', onStopLoading)
  })
...
function loadUrlInWindow(window, url) {
  ...
  var promise = waitUntilLoaded(window)
...
function loadUrl(url, doWork) {
  return acquireWindow().then(function (window) {
